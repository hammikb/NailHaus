...
function actionLabel(action: string) {
  if (action === 'promote_to_vendor') return 'Promoted a user to vendor';
  if (action === 'make_normal_user') return 'Changed an account to normal user';
  if (action === 'promote_to_admin') return 'Promoted a user to admin';
  if (action === 'disable_user') return 'Disabled a user account';
  if (action === 'enable_user') return 'Re-enabled a user account';
  if (action === 'verify_vendor') return 'Approved a vendor verification';
  if (action === 'reject_vendor') return 'Rejected a vendor verification';
  return 'Updated the marketplace';
}
...
export function AdminDashboardClient() {
  ...
  const loadUsers = useCallback(async () => {
    try {
      const rows = await api.getAdminUsers(userSearch, userRoleFilter, userStatusFilter);
      setUsers(rows);
  ...
  const refreshCurrentTab = useCallback(async () => {
    try {
      if (tab === 'verifications') setVerifications(await api.getVerificationRequests(verifFilter));
      if (tab === 'users') await loadUsers();
      if (tab === 'vendors') setVendors(await api.getAdminVendors(vendorSearch, vendorVerifiedFilter));
      if (tab === 'products') setProducts(await api.getAdminProducts(productSearch, productHiddenFilter));
      if (tab === 'orders') setOrders(await api.getAdminOrders(orderStatusFilter));
    } catch {
      if (tab === 'verifications') setVerifications([]);
      if (tab === 'vendors') setVendors([]);
  ...
  useEffect(() => { void refreshCurrentTab(); }, [refreshCurrentTab]);
    ...
    <main className="page-shell">
      <div className="container">
        ...
        <div className="tab-nav">
          {TABS.map((item) => (
            <button key={item} className={`tab-btn${tab === item ? ' active' : ''}`} onClick={() => setTab(item)}>
              {item === 'verifications' && stats?.pendingVerifications ? `Verifications (${stats.pendingVerifications})` : item.charAt(0).toUpperCase() + item.slice(1)}
            </button>
          ))}
        </div>
          ...
          <>
            <div className="grid" style={{ gridTemplateColumns: '1.2fr .9fr', gap: 18, marginBottom: 18 }}>
              <div className="panel" style={{ padding: 24 }}>
                ...
                <p className="muted" style={{ marginTop: 0 }}>This dashboard now supports role management, vendor promotion, verification review, moderation, and recent admin activity tracking.</p>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: 12 }}>
                  ...
                  <div className="panel" style={{ padding: 16, boxShadow: 'none' }}><div className="muted" style={{ fontSize: '.74rem', fontWeight: 800 }}>Verified vendors</div><div style={{ fontWeight: 800 }}>{stats.verifiedVendors}</div></div>
                  <div className="panel" style={{ padding: 16, boxShadow: 'none' }}><div className="muted" style={{ fontSize: '.74rem', fontWeight: 800 }}>Unverified vendors</div><div style={{ fontWeight: 800 }}>{stats.unverifiedVendors}</div></div>
                </div>
              </div>
              <div className="panel" style={{ padding: 24 }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  ...
                  <button className="pill btn-sm btn-ghost" onClick={() => setTab('products')}>Moderate products</button>
                </div>
              </div>
            </div>
            ...
            <div className="grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 18 }}>
              ...
              <div className="panel" style={{ padding: 22 }}>
                <div className="between" style={{ marginBottom: 12 }}><strong>Recent admin activity</strong><button className="pill btn-sm btn-ghost" onClick={() => setTab('users')}>Open moderation</button></div>
                {stats.recentAdminActions.length ? stats.recentAdminActions.map((entry) => <div key={entry.id} className="list-item"><div style={{ fontWeight: 700 }}>{actionLabel(entry.action)}</div><div className="muted" style={{ fontSize: '.8rem', marginTop: 4 }}>{entry.entityType.replace(/_/g, ' ')} #{entry.entityId.slice(0, 8) || 'n/a'} / {shortDate(entry.createdAt)}</div>{entry.note ? <div className="muted" style={{ fontSize: '.8rem', marginTop: 6 }}>{entry.note}</div> : null}</div>) : <div className="empty-state" style={{ padding: 24 }}><p>No admin actions yet.</p></div>}
              </div>
            </div>
          </>
        ) : null}
        ...
        {tab === 'verifications' ? (
          <>
            <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
              <button className={`filter-pill${verifFilter === 'pending' ? ' active' : ''}`} onClick={() => setVerifFilter('pending')}>Pending</button>
              <button className={`filter-pill${verifFilter === 'all' ? ' active' : ''}`} onClick={() => setVerifFilter('all')}>All</button>
            </div>
            {!verifications.length ? <div className="panel empty-state"><p>No verification requests found.</p></div> : verifications.map((request) => <div key={request.id} className="panel list-item" style={{ padding: 24 }}><div className="between"><div><strong>{request.vendors?.name || 'Unknown vendor'}</strong><div className="muted" style={{ fontSize: '.82rem', marginTop: 4 }}>Submitted by {request.profiles?.name || 'Unknown'} / {shortDate(request.created_at)}</div></div>{request.status === 'pending' ? <div style={{ display: 'flex', gap: 8 }}><button className="pill btn-sm" style={{ background: 'var(--success-bg)', color: 'var(--success)', borderColor: '#86efac' }} disabled={busy} onClick={async () => { setBusy(true); try { await api.reviewVerification(request.id, 'approved'); showToast('Vendor approved.'); await Promise.all([refreshCurrentTab(), loadStats()]); } finally { setBusy(false); } }}>Approve</button><button className="pill btn-sm btn-danger" disabled={busy} onClick={async () => { setBusy(true); try { await api.reviewVerification(request.id, 'rejected'); showToast('Request rejected.'); await Promise.all([refreshCurrentTab(), loadStats()]); } finally { setBusy(false); } }}>Reject</button></div> : <span className="chip">{request.status}</span>}</div>{request.message ? <p className="subtle" style={{ marginBottom: 0 }}>{request.message}</p> : null}</div>)}
          </>
        ) : null}
        ...
        {tab === 'users' ? (
          <>
            ...
            <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
              <div className="filter-search-wrap" style={{ flex: 1, minWidth: 240 }}><span className="filter-search-icon" aria-hidden="true">&#128269;</span><input className="filter-search" placeholder="Search by name or email..." value={userSearch} onChange={(e) => setUserSearch(e.target.value)} /></div>
            ...
            {!users.length ? <div className="panel empty-state"><p>No users found.</p></div> : <div className="panel" style={{ overflow: 'hidden' }}><table className="data-table"><thead><tr><th>User</th><th>Category</th><th>Vendor profile</th><th>Joined</th><th>Status</th><th>Actions</th></tr></thead><tbody>{users.map((account) => { const isSelf = account.id === user.id; const draftRole = roleDrafts[account.id] || (account.role as UserRole); return <tr key={account.id}><td><div style={{ fontWeight: 700 }}>{account.name}</div><div className="muted" style={{ fontSize: '.8rem' }}>{account.email || 'No email available'}</div></td><td><div style={{ display: 'grid', gap: 8 }}><span className="chip">{roleLabel(account.role)}</span><select className="field-input" style={{ minWidth: 150, padding: '8px 10px' }} value={draftRole} disabled={busy || isSelf} onChange={(e) => setRoleDrafts((current) => ({ ...current, [account.id]: e.target.value as UserRole }))}><option value="buyer">Normal User</option><option value="vendor">Vendor</option><option value="admin">Admin</option></select></div></td><td>{account.hasVendorProfile ? <div><div style={{ fontWeight: 700 }}>{account.vendorName || 'Vendor profile'}</div><div className="muted" style={{ fontSize: '.8rem' }}>{account.vendorVerified ? 'Verified' : 'Unverified'} / {account.totalProducts} products / {account.totalSales} sales</div></div> : <span className="muted" style={{ fontSize: '.85rem' }}>No vendor profile</span>}</td><td className="muted" style={{ fontSize: '.82rem' }}>{shortDate(account.created_at)}</td><td><span className="chip" style={{ background: account.disabled ? 'var(--danger-bg)' : 'var(--success-bg)', color: account.disabled ? 'var(--danger)' : 'var(--success)' }}>{account.disabled ? 'Disabled' : 'Active'}</span></td><td><div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}><button className={`pill btn-sm ${account.disabled ? '' : 'btn-danger'}`} style={account.disabled ? { background: 'var(--success-bg)', color: 'var(--success)', borderColor: '#86efac' } : {}} disabled={busy || isSelf} onClick={() => toggleUserDisabled(account)}>{account.disabled ? 'Re-enable' : 'Disable'}</button><button className="pill btn-sm btn-ghost" disabled={busy || isSelf || draftRole === account.role} onClick={() => updateUserRole(account)}>Save role</button></div></td></tr>; })}</tbody></table></div>}
          </>
        ) : null}
        ...
        {tab === 'vendors' ? (
          <>
            <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
              <div className="filter-search-wrap" style={{ flex: 1, minWidth: 240 }}><span className="filter-search-icon" aria-hidden="true">&#128269;</span><input className="filter-search" placeholder="Search vendors..." value={vendorSearch} onChange={(e) => setVendorSearch(e.target.value)} /></div>
            ...
            {!vendors.length ? <div className="panel empty-state"><p>No vendors found.</p></div> : <div className="panel" style={{ overflow: 'hidden' }}><table className="data-table"><thead><tr><th>Vendor</th><th>Products</th><th>Sales</th><th>Rating</th><th>Status</th><th>Action</th></tr></thead><tbody>{vendors.map((vendor) => <tr key={vendor.id}><td><strong>{vendor.name}</strong></td><td>{vendor.totalProducts ?? 0}</td><td>{vendor.totalSales ?? 0}</td><td>{vendor.rating ? Number(vendor.rating).toFixed(1) : '-'}</td><td><span className="chip">{vendor.verified ? 'Verified' : 'Unverified'}</span></td><td><button className="pill btn-sm btn-ghost" disabled={busy} onClick={async () => { setBusy(true); try { await api.toggleVendorVerified(vendor.id, !vendor.verified); showToast(vendor.verified ? 'Vendor verification removed.' : 'Vendor verified.'); await Promise.all([refreshCurrentTab(), loadStats()]); } finally { setBusy(false); } }}>{vendor.verified ? 'Remove verified' : 'Verify vendor'}</button></td></tr>)}</tbody></table></div>}
          </>
        ) : null}
        ...
        {tab === 'products' ? (
          <>
            <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
              <div className="filter-search-wrap" style={{ flex: 1, minWidth: 240 }}><span className="filter-search-icon" aria-hidden="true">&#128269;</span><input className="filter-search" placeholder="Search products..." value={productSearch} onChange={(e) => setProductSearch(e.target.value)} /></div>
