...
type ProductCardDensity = 'default' | 'compact' | 'ultra';
...
export function ProductCard({ product, density = 'default' }: { product: Product; density?: ProductCardDensity }) {
  ...
  const showVendor = density === 'default' || density === 'compact';
  const showShapeStyle = density === 'default';
  const showAvailability = density !== 'ultra';
  const showQuickView = density === 'default';
  const titleClassName = `product-title${density !== 'default' ? ` product-title-${density}` : ''}`;
  const metaClassName = `product-meta${density !== 'default' ? ` product-meta-${density}` : ''}`;
  const priceRowClassName = `product-price-row${density !== 'default' ? ` product-price-row-${density}` : ''}`;
  const priceClassName = `price${density !== 'default' ? ` price-${density}` : ''}`;

  return (
    <>
      <div
        className={`card product-card product-card-${density}`}
      ...
      >
        <Link href={`/products/${product.id}`} style={{ display: 'contents' }}>
          ...
          <div className="card-body">
            ...
            <div className={titleClassName}>{product.name}</div>
            ...
            <div className={metaClassName}>
              ...
              {showShapeStyle && <span className="muted" style={{ fontSize: '.76rem' }}>· {product.shape} · {product.style}</span>}
            </div>
